...
class DynamicProgramming:
    ...
    def compute_episode_rewards(self, num_episodes=100, step_limit=1000):
        ...
        steps_taken = np.zeros(num_episodes)
    ...
    def print_rewards_info(self, num_episodes=100, step_limit=1000):
        ...
        """
        mean, var, best, steps_array = self.compute_episode_rewards(
            num_episodes=num_episodes, step_limit=step_limit
        )
        print(
            f"Mean of Episode Rewards: {mean:.2f}, Variance of Episode Rewards: {var:.2f}, Best Episode Reward: {best}"
        )
        ...
        for episode in range(num_episodes):
            ...
            while not finished and num_steps < step_limit:
                best_actions = np.argwhere(
                    self.Q[curr_state] == np.amax(self.Q[curr_state])
                ).flatten()
...
if __name__ == "__main__":
    ...
    mean_val, var_val, max_val, num_steps_array = my_policy.compute_episode_rewards(
        num_episodes=1000, step_limit=1000
    )
    print(
        f"Mean of Episode Rewards: {mean_val:.2f}, Variance of Episode Rewards: {var_val:.2f}, Best Episode Reward: {max_val}"
    )
