...
if __name__ == "__main__":
    ...
    y_train = fetch_np_array(
        "http://yann.lecun.com/exdb/mnist/train-labels-idx1-ubyte.gz", pset_5_cache
    )[8:]
    X_test = fetch_np_array(
        "http://yann.lecun.com/exdb/mnist/t10k-images-idx3-ubyte.gz", pset_5_cache
    )[0x10:].reshape((-1, 28, 28))
    y_test = fetch_np_array(
        "http://yann.lecun.com/exdb/mnist/t10k-labels-idx1-ubyte.gz", pset_5_cache
    )[8:]

    # Initialize tensorflow model for training and evaluation
    tf_model = TensorFlowMNISTClassifier()
    tf_accuracy = train_and_evaluate_mnist_model(
        tf_model, X_train, y_train, X_test, y_test
    )
